  ...
  <main id="main">
    ...
    <section id="confirmPost">
	  ...
	  <div class="container">
		...
		"SELECT tblblogpost.postID, tbluser.username, tblblogpost.heading, tblblogpost.postDate 
		...
		AND tblconfirmedposts.confirmed=0
		AND tbluser.userID 
		NOT IN(SELECT tbluser.userID FROM tbluser, tbladmin WHERE tbluser.userID=tbladmin.userID) 
		ORDER BY tblblogpost.postDate DESC")){
			...
			//get results
			mysqli_stmt_bind_result($stmt, $postID, $username, $heading, $date);
			...
			while(mysqli_stmt_fetch($stmt)){
				echo"
					<tr>
						...
						<td>$heading</td>
						<td>$date</td>
						<td><a href='confirmPost.php?postID=$postID'>
						<button class='btn btn-outline-primary form-control sincerely'>View</button></a>
						</td>
					</tr>
				";
			}//end of while
